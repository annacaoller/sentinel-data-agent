...
The CLI layer exposes commands such as sentinel run, sentinel status, and sentinel report.

The Orchestrator layer coordinates the execution of capabilities using a DAG-based execution plan.

The Capabilities layer implements deterministic analysis steps such as schema detection, correlation analysis, and baseline modeling.

The State layer persists run state, artifacts, and checkpoints.
...
Each phase produces artifacts stored on disk and referenced in the run state.

Architecture Diagram

The following diagram shows how Sentinel components interact during a run.
...
run_id: sda_20260308_221443_870_32972
...
Capabilities tests validate schema detection, target inference, exploratory analysis, correlation analysis, feature readiness evaluation, baseline modeling, and final report generation.

The orchestrator test validates full pipeline execution.

The CLI test validates end-to-end execution using the command line interface.
...
Sentinel was designed around several engineering principles that prioritize reliability and transparency.
...
Planned extensions include support for regression pipelines, automated feature engineering, richer dataset quality diagnostics, visualization reports, and optional LLM-based analysis layers.
